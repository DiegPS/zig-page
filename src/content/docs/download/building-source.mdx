---
title: Building Zig from source
description: Instructions for building Zig from source
---
...
If your goal is to install a specific version of Zig, you can find pre-built tarballs on [the download page](/download/). You could also try [installing Zig from a package manager](/download/package-manager/). Finally, there is [zig-bootstrap](https://github.com/ziglang/zig-bootstrap) to cross-compile an installation of Zig from source for any target. When using zig-bootstrap, be sure to check out the git tag corresponding to the version you want to build, as master branch is not kept in any coherent state.
